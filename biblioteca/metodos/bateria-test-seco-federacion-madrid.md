# Batería de test en seco — pruebas de nivel (Federación Madrileña, natación artística)

**Fuente:** `biblioteca/fuentes/natacion-artistica/requisitos-alevin-1.pdf`, `requisitos-alevin-2.pdf`,
...
Estas pruebas **no son una propuesta: son el filtro oficial de apto/no apto por nivel**. El trabajo de
preparación física en seco tiene un objetivo medible y ya definido — que las atletas pasen estas
pruebas y las superen con margen. Cualquier programación que no las tenga como diana está trabajando
a ciegas.

Y la observación profesional relevante: **la batería es calistenia y gimnasia**. Dominadas con dos
agarres, verticales sobre tres apoyos, plancha, hollow, puente, pino-puente-remontada, espagats con
tolerancias en centímetros. Es exactamente el dominio de Daniel — la transferencia es directa, no
analógica.
...
1. **Es el test de entrada y el de salida.** Se pasa al inicio de temporada para saber dónde está cada
   atleta, y se repite en los momentos que marque la programación. Los resultados van a
   `frentes/natacion-artistica/privado/` con códigos (`ATL-01`), nunca con nombres.
2. **Cada criterio de "no apto" es un objetivo de entrenamiento con nombre propio.** Si una atleta
   falla el puente por 20° de brazo, el trabajo es movilidad de hombro en flexión, no "más puente".
3. **Ojo con el sesgo de la prueba:** son criterios de apto/no apto, no una medida continua de
   rendimiento. Para ver progreso dentro del "no apto" hace falta registrar la magnitud (cm al suelo,
   grados, segundos, repeticiones), no solo el binario. Eso es lo que permite mostrar mejora a mitad de
   camino, y mantener la motivación.
4. **Transferencia a otros frentes:** las progresiones de dominada, vertical, plancha y hollow son las
   mismas que usas en calistenia. Lo que construyas aquí sirve en
   [entrenador-calistenia](../../frentes/entrenador-calistenia/ESTADO.md).
